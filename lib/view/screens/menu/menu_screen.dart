...
class MenuScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if(Get.find<SplashController>().configModel!.refundPolicyStatus == 1 ){
      menuList.add(MenuModel(icon: Images.refund, title: 'refund_policy'.tr, route: RouteHelper.getHtmlRoute('refund-policy')));
    }
    if(Get.find<SplashController>().configModel!.cancellationPolicyStatus == 1 ){
      menuList.add(MenuModel(icon: Images.cancellation, title: 'cancellation_policy'.tr, route: RouteHelper.getHtmlRoute('cancellation-policy')));
    }
    if(Get.find<SplashController>().configModel!.shippingPolicyStatus == 1 ){
      menuList.add(MenuModel(icon: Images.shippingPolicy, title: 'shipping_policy'.tr, route: RouteHelper.getHtmlRoute('shipping-policy')));
    }
    if(Get.find<SplashController>().configModel!.refEarningStatus == 1 ){
      menuList.add(MenuModel(icon: Images.referCode, title: 'refer'.tr, route: RouteHelper.getReferAndEarnRoute()));
    }
    if(Get.find<SplashController>().configModel!.customerWalletStatus == 1 ){
      menuList.add(MenuModel(icon: Images.wallet, title: 'wallet'.tr, route: RouteHelper.getWalletRoute(true)));
    }
    if(Get.find<SplashController>().configModel!.loyaltyPointStatus == 1 ){
      menuList.add(MenuModel(icon: Images.loyal, title: 'loyalty_points'.tr, route: RouteHelper.getWalletRoute(false)));
    }
